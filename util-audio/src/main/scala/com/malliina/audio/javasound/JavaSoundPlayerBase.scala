...
trait JavaSoundPlayerBase extends RichPlayer with Seekable:
  ...
  /** Bug: DataLine.getMicrosecondPosition returns milliseconds on Oracle's ARM JVM.
    *
    * To workaround the bug, this method conditionally converts what may be milliseconds to
    * microseconds. However, that's based on guessing, so when the position is incorrectly reported
    * as milliseconds and the position is over 1000 seconds, no conversion takes place and
    * milliseconds are incorrectly returned.
    *
    ...
    *
    * @return
    *   microseconds since the line was opened
    */
  ...
  def volume_=(newVolume: Int): Unit =
    ...
    else if hasGainControl then gainControlValue(1.0f * newVolume / 100)
    else
  ...
  /** This throws an [[IllegalArgumentException]] after end of media. So I think that if the
    ...
    *
    * @param controlType
    *   type of control: volume, mute, ...
    * @tparam T
    *   actual type of control
    * @return
    *   the control
    */
  ...
  private def dbValue(normalizedValue: Float, gainControl: FloatControl) =
    ...
    val minDbGain = gainControl.getMinimum
    val posDbFactor = maxDbGain / (maxGain - zeroGain)
    if normalizedValue >= zeroGain then (normalizedValue - zeroGain) * posDbFactor
    else (zeroGain - normalizedValue) * minDbGain / zeroGain
