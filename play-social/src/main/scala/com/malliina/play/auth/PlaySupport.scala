...
trait PlaySupport[U]:
  ...
  /** Adds a random state parameter and an optional nonce and returns a redirect to the
    * authorization URL with all parameters set.
    *
  ...
  def redirResult(
    ...
    val encodedParams = (authParams ++ Map(State -> state)).map:
...
trait PlayFlow[U] extends PlaySupport[U] with FlowStart[IO]:
  self: CallbackValidator[IO, U] =>
