...
/** A WebSocket client.
  ...
  *
  * Supports automatic reconnections. Calling `close()` will close any open resources and cancel
  * future reconnections, after which this instance must no longer be used.
  */
class SocketClient(
  val uri: URI,
  ...
  private val loop = loopExecutor.scheduleWithFixedDelay(
    new Runnable:
  ...
  private val listener = new WebSocketAdapter:
    ...
    override def handleCallbackError(websocket: WebSocket, cause: Throwable) =
      super.handleCallbackError(websocket, cause)

    override def onConnected(websocket: WebSocket, headers: util.Map[String, util.List[String]]) =
  ...
  private def createNewSocket(): WebSocket =
    ...
    headers.foreach: header =>
      socket.addHeader(header.key, header.value)
  ...
  private def ensureConnected() =
    if isEnabled then
      if !isConnected then reconnect()
    else log.warn(s"Socket to $uri is no longer enabled.")
