...
/** Methods that choose the correct response to provide to clients based on what they accept
  * (HTML/JSON/which JSON version).
  */
trait PimpContentController extends PlayCirce:

  import JsonFormatVersions.*

  def pimpResponse(
    request: RequestHeader
  )(html: => Result, json17: => Json, latest: => Json): Result =
  ...
  def respond[C: Writeable, T: Encoder](
    request: RequestHeader
  )(html: => C, json: => T, status: Status = Ok): Result =
    pimpResult(request)(status(html), status(json))

  def pimpResponseOkJson(request: RequestHeader)(html: => Result, json: => Json): Result =
    pimpResult(request)(html, Ok(json))

  /** @return
    *   the equivalent of "Unit" in JSON and HTML
    */
  ...
  def pimpResult2(
    request: RequestHeader
  )(html: => Future[Result], json: => Result): Future[Result] =
...
object PimpContentController:
  ...
  def pimpResultAsync(
    request: RequestHeader
  )(html: => Future[Result], json: => Future[Result]): Future[Result] =
    PimpRequest.requestedResponseFormat(request) match
      case Some(MimeTypes.HTML)                     => html
      case Some(format) if format.contains(JsonKey) => json
