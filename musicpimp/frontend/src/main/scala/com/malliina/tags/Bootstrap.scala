...
trait Bootstrap2 {
  ...
  def fullRow(inner: Modifier*) = rowColumn(ColMd12)(SeqNode(inner))

  def halfRow(inner: Modifier*) = rowColumn(ColMd6)(SeqNode(inner))

  def rowColumn(clazz: String)(inner: Modifier*) = row(divClass(clazz)(SeqNode(inner)))
  ...
  def blockSubmitButton(more: Modifier*) =
    submitButton(`class` := s"$BtnPrimary $BtnBlock", SeqNode(more))
  ...
  def responsiveTable[T](entries: Seq[T])(headers: String*)(cells: T => Seq[Modifier]) =
    headeredTable(TableStripedHoverResponsive, headers.map(stringFrag))(
      tbody(entries.map(entry => tr(SeqNode(cells(entry)))))
    )
}
