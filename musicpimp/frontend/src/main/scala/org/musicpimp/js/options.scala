...
object StopOptions {
  def default(stop: (Event, Ui) => Unit) =
    apply(stop)
...
object SliderOptions {
  ...
  def apply(
    ...
    min: Int,
    max: Int,
    stop: js.Function2[Event, Ui, Any]
  ) =
    js.Dynamic
      .literal(orientation = orientation, range = range, min = min, max = max, stop = stop)
      .asInstanceOf[SliderOptions]
}
