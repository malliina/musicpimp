...
object SaveCmd {
  val Key = "save"
  implicit val json: OFormat[SaveCmd] = cmd(Key, Json.format[SaveCmd])
}

case class DeleteCmd(id: String) extends AlarmCommand

object DeleteCmd {
  implicit val json: OFormat[DeleteCmd] = cmd(Delete.Key, Json.format[DeleteCmd])
}

case class StartCmd(id: String) extends AlarmCommand

object StartCmd {
  implicit val json: OFormat[StartCmd] = cmd(Start.Key, Json.format[StartCmd])
}
...
object AddWindowsDevice {
  val Key = "push_add"
  implicit val json: OFormat[AddWindowsDevice] = cmd(Key, Json.format[AddWindowsDevice])
}
...
object RemoveWindowsDevice {
  val Key = "push_remove"
  implicit val json: OFormat[RemoveWindowsDevice] = cmd(Key, Json.format[RemoveWindowsDevice])
}
...
object RemovePushTag {
  val Key = "push_remove"
  implicit val json: OFormat[RemovePushTag] = cmd(Key, Json.format[RemovePushTag])
}
...
object AddGoogleDevice {
  val Key = "gcm_add"
  implicit val json: OFormat[AddGoogleDevice] = cmd(Key, Json.format[AddGoogleDevice])
}
...
object RemoveGoogleDevice {
  val Key = "gcm_remove"
  implicit val json: OFormat[RemoveGoogleDevice] = cmd(Key, Json.format[RemoveGoogleDevice])
}
...
object AddAmazonDevice {
  val Key = "adm_add"
  implicit val json: OFormat[AddAmazonDevice] = cmd(Key, Json.format[AddAmazonDevice])
}
...
object RemoveAmazonDevice {
  val Key = "adm_remove"
  implicit val json: OFormat[RemoveAmazonDevice] = cmd(Key, Json.format[RemoveAmazonDevice])
}
...
object AddApnsDevice {
  ...
  val Key = "apns_add"
  implicit val json: OFormat[AddApnsDevice] = cmd(Key, Json.format[AddApnsDevice])
}
...
object RemoveApnsDevice {
  val Key = "apns_remove"
  implicit val json: OFormat[RemoveApnsDevice] = cmd(Key, Json.format[RemoveApnsDevice])
}

case object StopPlayback extends AlarmCommand {
  implicit val json: OFormat[StopPlayback.type] = singleCmd(Stop.Key, StopPlayback)
}

object AlarmCommand {
  implicit val reader: Reads[AlarmCommand] = Reads[AlarmCommand] { json =>
    DeleteCmd.json
      .reads(json)
