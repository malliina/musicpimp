...
object PushUrls extends FileSet[PushUrl]("push.json"):
  ...
  /** The same device may open different push URLs at different points in time, however the old ones
    ...
    * however, otherwise the same push notification is sent multiple times to the same device.
    * Therefore we need another way of uniquely identifying a device, and that is the tag.
    *
    * @param elem
    *   data sent by device
    * @return
  ...
  def removeURL(url: String): Unit =
    get().find(_.url.token == url).foreach(elem => removeID(id(elem)))
