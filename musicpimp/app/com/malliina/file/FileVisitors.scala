...
object FileVisitors:
  ...
  abstract class FileCollectingVisitor extends SimpleFileVisitor[Path]:
    ...
    /** @return
      *   the files that have been visited and passed any filters
      */
  ...
  /** File visitor that adds visited regular files to a buffer; comes with an optional filter that
    * subclasses can implement; by default all files are included.
    */
  class FilteringFileVisitor extends FileCollectingVisitor:
    override def visitFile(file: Path, attrs: BasicFileAttributes) =
      if attrs.isRegularFile && qualifies(file, attrs) then add(file)
      FileVisitResult.CONTINUE

    /** Override to filter files as you wish. Qualified files can be queried using the
      ...
      * @return
      *   true if the file qualifies, false otherwise
      */
  ...
  /** Filtering file visitor for old files.
    *
    * @param ageLimitHours
    *   the threshold age in hours: files with an older timestamp, as defined by
    *   <code>comparisonTimestamp</code>, will qualify for inclusion
    */
  ...
  /** Visitor for directory traversals. Visited files and directories are available in the
    * <code>paths</code> member.
    */
  ...
  trait TimestampComparison:
    ...
    /** A file timestamp used for comparisons in the file filter.
      ...
      *   file attrs
      * @return
      *   a timestamp in milliseconds since the epoch, usually the create or last modified timestamp
      */
