...
//    val subscription = connectable.connect
  ...
  def fromFuture[T](body: => T)(implicit executor: ExecutionContext): Observable[T] =
    Observable.from(Future(body))
  ...
  def timeoutAfter[T](duration: Duration, promise: Promise[T]) =
    after(duration)(
      promise tryFailure new concurrent.TimeoutException(s"Timed out after $duration."))
