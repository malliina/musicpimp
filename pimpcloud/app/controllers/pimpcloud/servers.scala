...
class Servers(phoneMediator: ActorRef, val ctx: ActorExecution, errorHandler: HttpErrorHandler):
  ...
  /** The server must authenticate with Basic HTTP authentication. The username must either be an
    * empty string for new clients, or a previously used cloud ID for old clients.
    *
    * @param rh
    *   headers
    * @return
