...
class Phones(comps: ControllerComponents, tags: CloudTags, phoneAuth: BaseSecurity[PhoneConnection])
  ...
  protected def customProxied(
    cmd: String
  )(body: Request[JsValue] => Either[String, JsObject]): EssentialAction =
  ...
  private def proxiedJsonAction[C: Writes](
    cmd: String
  ...
  private def executeOkProxied[A, W: Writes](
    parser: BodyParser[A]
  ...
  private def onGatewayParseErrorResult(
    err: collection.Seq[(JsPath, collection.Seq[JsonValidationError])]
  ) = {
