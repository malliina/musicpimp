...
class StreamReceiver(comps: ControllerComponents) extends AbstractController(comps):
  ...
  def receiveStream(
    ...
    Action(composedParser): (parsedRequest: Request[MultipartFormData[Long]]) =>
  ...
  /** This is not strictly necessary, but cleans up ugly and confusing stacktraces if the client
    * disconnects while an upload is in progress.
    */
