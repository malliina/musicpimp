        ...
        divClass(s"${col.md.four} wrapper login-container")(
          ...
          row(
            form(`class` := FormSignin,
              ...
              button(`type` := Submit,
                     id := "loginbutton",
                     `class` := s"${btn.primary} ${btn.lg} ${btn.block}")("Sign in")
            )
          ),
  ...
  def trackActions(track: TrackID) =
    divClass(btn.group)(
      a(`class` := s"${btn.default} ${btn.sm} $PlayLink", href := "#", id := s"play-$track")(
        iconic("media-play"),
        " Play"),
      a(`class` := s"${btn.default} ${btn.sm} $DropdownToggle",
        dataToggle := Dropdown,
        href := "#")(spanClass(Caret)),
      ulClass(DropdownMenu)(
        li(
          a(href := "#", `class` := PlaylistLink, id := s"add-$track")(iconic("plus"),
                                                                       " Add to playlist")),
        li(
          a(href := routes.Phones.track(track), download)(iconic("data-transfer-download"),
                                                          " Download"))
      )
    )
