...
import com.malliina.musicpimp.models.{FolderID, Ident, TrackID}

object PimpEnc {
  val UTF8 = "UTF-8"

  def relativePath(itemId: Ident): Path = Paths get decode(itemId)
  ...
  def decode(trackID: Ident) = URLDecoder.decode(trackID.id, UTF8)
}
