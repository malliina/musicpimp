        ...
        .map: savedToken =>
          if savedToken.token == attempt.token then
            ...
            /** I believe the intention is to ensure that a browser cannot reuse another browser's
              * token.
              *
              * The token is replaced with a new one at each successful token authentication, while
              * the series remains the same; this updated cookie is then sent to the browser. The
              * series acts as a browser identifier. So, if there's a token mismatch, it suggests
              * some other actor has authenticated using this browser's token, which is suspect.
              */
