...
class RememberMe(
  ...
  /** The API says we must return a token, even if deserialization fails, so we introduce the
    * concept of an "empty" token and filter it away in `readToken(RequestHeader)`.
    *
    * @param data
    *   token data
    * @return
  ...
  /** @return
    ...
    */
  def authenticateToken(req: RequestHeader): IO[Option[Token]] =
    authenticate(req).map(_.toOption)

  def authenticate(req: RequestHeader): IO[Either[AuthFailure, Token]] =
    readToken(req)
      .map(t => cookieAuth(t, req))
      .getOrElse:
        log.debug(s"Found no token in request: ${req.cookies}")
        IO.pure(Left(MissingCookie(req)))

  def cookify(token: Token) = encodeAsCookie(token.asUnAuth)

  def persistNewCookie(loggedInUser: Username): IO[Cookie] =
    createToken(loggedInUser).map(cookify)

  private def createToken(loggedInUser: Username): IO[Token] =
