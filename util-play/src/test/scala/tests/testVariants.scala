...
trait PlayAppFixture[T <: BuiltInComponents]:
  self: FunSuite =>
  ...
  val app = FunFixture[T](
    opts =>
      ...
      components
    ,
    comps => Play.stop(components.application)
  )

trait PlayServerFixture[T <: BuiltInComponents]:
  self: FunSuite =>
  def components(context: Context): T
  lazy val components: T = components(TestAppLoader.createTestAppContext)
  val server = FunFixture[RunningServer](
    opts => DefaultTestServerFactory.start(components.application),
    running => running.stopServer.close()
  )

trait AppPerSuite[T <: BuiltInComponents]:
  self: Suite =>
...
trait ServerPerSuite[T <: BuiltInComponents]:
  self: Suite =>
  def createComponents(context: Context): T
  lazy val components = createComponents(TestAppLoader.createTestAppContext)
